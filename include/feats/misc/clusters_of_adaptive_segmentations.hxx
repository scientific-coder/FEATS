...
struct clusters_of_adaptive_segmentations {
  ...
  typedef AlgoType<seg_type, int> algo_type;
...
maintenant c'est segmentations_for_total_number avec des segmentations utilisant des models_segments */
  ...
  typedef SegmentationsType<algo_type> segmentations_type;
     ...
     , clusters_(nbSeries_),nbIter_(0)
  {
	...
	km.output_prototype(i,prototypes_[i].begin());
      ...
      /* reverted
      kmeans_type* kmPtr=new kmeans_type(boost::make_tuple(vipip, nbPrototypes_));
      kmPtr->output_clusters(clusters_.begin(), true); // we update the prototype
      for(size_type i(0); i!=nbPrototypes_; ++i){
	kmPtr->output_prototype(i,prototypes_[i].begin());
      }
      */
    }
