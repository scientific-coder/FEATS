...
struct croeuc {
    ...
    ,segmentationCost_(std::numeric_limits<cost_type>::max()),clusteringCost_(std::numeric_limits<cost_type>::max())
  {
    ...
    kmp->output_sizes(std::ostream_iterator<size_type>(std::cout," "));
    std::cout<<'\n';
    std::cerr<<"\ninitial prototypes:\n";
    for(size_type i(0); i!=nbPrototypes_; ++i){kmp->output_prototype(i,prototypes_[i].begin());
      //   kmp->output_prototype(i,std::ostream_iterator<value_type>(std::cerr," "));std::cerr<<std::endl; 
}
  ...
  cost_type next_segmentation() {
    ...
    protos_seq prototypesModelsToSeg;
    std::cerr<<"computing models for segmentation\n";
    for(size_type i(0); i!= seriesLength_; ++i){
      model_type tmp;
      for(size_type j(0); j!=nbPrototypes_; ++j){
	if(clustersSize_[j]){
	  model_type toAdd(feats::segmentations::make_weighted_value(prototypes_[j][i],clustersSize_[j]));
	  std::cerr<<"\nadding "<<prototypes_[j][i]<<"weight "<<clustersSize_[j];
	  tmp+=toAdd;
	}
      }
      std::cerr<<"model "<<i<<" is "<<boost::get<0>(tmp())<<" with cost "<<tmp.cost()<<"\n";
      prototypesModelsToSeg.push_back(tmp);
    }
    // do segmentation
    {
      seg_type initSeg(prototypesModelsToSeg.begin(), prototypesModelsToSeg.end());
