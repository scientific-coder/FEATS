#ifndef FEATS_SEGMENTATIONS_PROTOTYPES_SEGMENTATION_HXX
#define FEATS_SEGMENTATIONS_PROTOTYPES_SEGMENTATION_HXX
...
namespace feats{
  namespace segmentations{
    ...
    template<typename meta_model, template<typename, typename> class Segmentation > struct parameter_quantizer{
      ...
      parameter_quantizer(const boost::tuple<In,In>& modelsBeginEnd):models_(boost::get<0>(modelsBeginEnd)
	...
	return std::transform(tmp.begin(), tmp.end(), out, get_parameter_from_segment());
      ...
      ~parameter_quantizer(){delete seg_;}
    private:
...
I have a problem because prototype models templates depend on the type of the iterator over prototype parameters the later being decided by prototypes_segmentations, so prototypes_segmentation cannot receive the prototype_model class as parameter. 
    ...
    template<template<typename,typename, typename > class ProtoModelT, typename Iter, template<typename, typename >class Segmentation=binary_splits, template<typename, typename >class QuantizerSegmentation=binary_splits> struct prototypes_segmentations {
      typedef Iter serie_iterator;
      typedef typename std::iterator_traits<serie_iterator>::value_type data_type;
      typedef typename prototypes_model_traits<ProtoModelT,data_type,int>::free_model free_model;
      typedef typename prototypes_model_traits<ProtoModelT,data_type,int>::meta_model meta_model;
      typedef segment<free_model, serie_iterator> free_segment;
      typedef Segmentation<free_segment, int > free_segmentation_type;
      typedef parameter_quantizer<meta_model, QuantizerSegmentation> quantizer;
      ...
      prototypes_segmentations(const serie_boundaries_type& begEnd)
	:beginEnd_(begEnd),freeSeg_(init_segments<free_segmentation_type>()(begEnd))
			      //	 , ptrQuantizer_(0)
	 ,protoParams_(0)
			      //	 , ptrProtoSeg_(0)
{}
      ...
      free_segmentation_type& get_free_segmentation(){invalidate_protos(); return freeSeg_;}

      const quantizer& get_parameter_quantizer()const{ return *ensure_quantizer();}
      quantizer& get_parameter_quantizer(){ invalidate_segmentation(); return *ensure_quantizer();}
      
      const prototypes_segmentation& get_segmentation()const{return *ensure_segmentation();}
      prototypes_segmentation& get_segmentation(){return *ensure_segmentation();}
	...
	// note that we have to compute free_model s over thoses segments because the prototype models do not have the partial sums taht we need
	{
	  ...
	  invalidate_protos();
	  ptrQuantizer_= boost::shared_ptr<quantizer>(new quantizer(boost::make_tuple(boost::make_transform_iterator(tmp.begin(), free_model_from_proto_segment())
									    , boost::make_transform_iterator(tmp.end(), free_model_from_proto_segment()))));
	...
	if(!ptrQuantizer_){
	  ...
	  ptrQuantizer_=boost::shared_ptr<quantizer>(new quantizer(boost::make_tuple(boost::make_transform_iterator(tmp.begin(), boost::bind(&free_segment::model,_1))
										     , boost::make_transform_iterator(tmp.end(), boost::bind(&free_segment::model,_1)))));
	}
	...
	if(!ptrProtoSeg_){
	  ...
	  ptrProtoSeg_=boost::shared_ptr<prototypes_segmentation>(new prototypes_segmentation(initS(beginEnd_, proto_model(protoParams_.begin(), protoParams_.end()))));
