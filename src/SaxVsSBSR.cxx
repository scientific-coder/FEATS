...
#endif
/*
  en fait les distances sont calsulées par rapport aux moyennes comme APCA, la difference étant que lon peut aggréger les moyennes, pour gagner de la place, par symboles, sur chaque serie ou sur l'ensemble des séries
*/
...
dataType ratio(dataType d1, dataType d2){
  return (d2>std::numeric_limits<double>::min())? (d1/d2): 1.;
}
  ...
  for( int nbSegs(1); nbSegs!= boost::get<1>(nbSegsMinMax); ++nbSegs,protoSegs.free_segmentation().inc_segments()){
	...
	if(nbProtos>=boost::get<0>(nbProtosMinMax)){
	  ...
	  for(int nbIt(0); nbIt!=2; ++nbIt){
	    ...
	    saxSizes(nbSegs, std::distance(boost::get<0>(dataBegEnd), boost::get<1>(dataBegEnd)), sizes.begin());
	    ...
	    for(int i(0); i!= episodesDistsSAX.size(); ++i){
	      data_type tmp(sqrt(trueDists[i]));
	      sumWD+=ratio(sqrt(worstDists[i]),tmp); sumWDS+=ratio(sqrt(worstDistsSAX[i]),tmp);
	      sumGD+=ratio(sqrt(globalDists[i]),tmp); sumGDS+=ratio(sqrt(globalDistsSAX[i]),tmp);
	      sumLD+=ratio(sqrt(localDists[i]),tmp); sumLDS+=ratio(sqrt(localDistsSAX[i]),tmp);
	      sumED+=ratio(sqrt(episodesDists[i]),tmp); sumEDS+=ratio(sqrt(episodesDistsSAX[i]),tmp);
	      sumTD+=ratio(sqrt(trueDists[i]),tmp);
	    ...
	    std::cout<<sumWD/episodesDistsSAX.size()<<' '<<sumWDS/episodesDistsSAX.size()<<' '<<sumGD/episodesDistsSAX.size()<<' '<<sumGDS/episodesDistsSAX.size()<<' '<<sumLD/episodesDistsSAX.size()<<' '<<sumLDS/episodesDistsSAX.size()<<' '<<sumED/episodesDistsSAX.size()<<' '<<sumEDS/episodesDistsSAX.size()<<' '<<sumTD/episodesDistsSAX.size()<<std::endl;
